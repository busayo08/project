...
<div class="card">
        ...
        <h5 class="card-header info-color white-text text-center py-4 bg-warning">
            <strong>Delete Meal</strong>
        </h5>
    
        <!--Card content-->
                <?php
                if(isset($_GET['delete'])){
                    $sql = "DELETE FROM menu WHERE id=".$_GET['delete']."";   
                    $result60 = mysqli_query($link, $sql) or die ("Couldn't execute sql");
                if ($result60)  
                    echo'<div class="card-body px-lg-5 pt-0" style="height:500px;">
                    <h5 class="card-header white-text text-center py-4 text-warning">
                        <strong>Your Menu have been deleted successfully<br> To go back to Dashboard <a href="admin_dashboard.php">Click Here</a></strong>
                    </h5>';
                }
                ?>            
